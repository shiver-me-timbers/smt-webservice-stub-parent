...
public class ITJsonStub {
    ...
    @Test
    public void Can_stub_a_JSON_request() {
        ...
        // Given
        given.request(jsonReq(request)).willRespond(jsonRes(expected));

        // When
        final TestResponse actual = client.target(stubUrl.toString()).request(APPLICATION_JSON_TYPE)
            .post(json(request), TestResponse.class);

        // Then
        assertThat(actual, equalTo(expected));
        then.request(jsonReq(request)).should().beCalled();
    }

    @Test
    public void Can_fail_to_match_a_stubbed_a_JSON_request() {
        ...
        final Throwable actual = catchThrowable(
            () -> client.target(stubUrl.toString()).request(APPLICATION_JSON_TYPE)
                .post(json(new TestObject(someString(), child)), TestResponse.class)
        );
